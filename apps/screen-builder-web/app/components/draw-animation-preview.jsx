...
function getStepState(step, phaseMs) {
  ...
  const progress = phaseMs <= start ? 0 : phaseMs >= start + duration ? 1 : (phaseMs - start) / duration
    ...
    return (
      ...
      >
        <rect
          ...
          width={width}
          height={height}
          rx={Math.min(10, Math.round(Math.min(width, height) / 3))}
...
export default function DrawAnimationPreview({ screen, compact = false }) {
  ...
  return (
    <div className={`draw-preview-shell ${compact ? 'compact' : ''}`}>
      <div className="draw-preview-head">
        <div>
          <div className="text-[11px] uppercase tracking-[0.14em] text-ink-dim">Motion Canvas</div>
          <div className="text-xs text-ink-dim">{screen?.title || screen?.id || 'draw screen'}</div>
        </div>
