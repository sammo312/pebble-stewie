...
struct Stm32Exti {
    ...
    /* Pointer to GPIO array - set via board-level wiring */
...
/* Call when the EXTI should be triggered */
...
void stm32_exti_set_gpio(Stm32Exti *s, unsigned exti_line, const uint8_t gpio_index)
{
    ...
    f2xx_gpio_exti_set(s->stm32_gpio[gpio_index], exti_line,
                       qdev_get_gpio_in(DEVICE(&s->parent_obj), exti_line));
}
...
static void stm32_exti_realize(DeviceState *dev, Error **errp)
{
    int i;

    Stm32Exti *s = STM32_EXTI(dev);

    memory_region_init_io(&s->iomem, OBJECT(dev), &stm32_exti_ops, s,
            "exti", 0x03ff);
    ...
    qdev_init_gpio_in(dev, stm32_exti_gpio_in_handler, EXTI_LINE_COUNT);
}

/* Setter for GPIO array pointer — called by SoC/board code before realize */
...
static const TypeInfo stm32_exti_info = {
