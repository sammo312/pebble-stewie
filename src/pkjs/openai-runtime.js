...
function buildSystemPrompt(latestSchemaVersion) {
  return [
    ...
    'Use runtime context, especially watch capabilities and interaction preferences.',
    'Schema:',
    '{',
    ...
    '- run.type may be navigate, set_var, store, agent_prompt, agent_command, effect, or dictation',
    '- agent_command command may be reset or more_replies',
    ...
    '- if watch.supportsDictation is false, do not use voice or menu_or_voice input modes',
    '- if watch.preferTapInput is true, prefer concise tap-friendly choices over open-ended text entry',
    '- if watch.preferSuggestedReplies is true, prefer short suggested replies and include a path to more replies when space allows',
    '- if asked to generate more replies, return a menu screen with 3-5 concise, varied items',
    '- when returning reply suggestions, set each item value to the exact text the user should send next',
